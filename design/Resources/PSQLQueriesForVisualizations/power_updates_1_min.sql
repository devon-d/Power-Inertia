WITH myconstants (p_id, enddatetime) as (values (1, '2023-04-13T19:21:00'::timestamp )) 
    SELECT 
    -- GUAGE INFORMATION --
        -- Microgrid Overview Guage --
      mr.read_time at time zone 'utc' at time zone 'america/Los_angeles' AS read_time
    , p.id                                                               AS product_id 
    ...
    -- Battery Overview Guage
     ...
     , CEIL(br.soc_tenth *.1)                                AS "battery_soc"
    ...
    -- , p.<<EV CHARGER CAPACITY>>                      AS "ev_charger_capactiy" 
     FROM myconstants, meter_readings AS mr
               ...
               INNER JOIN (SELECT 
                             ...
                             AND date_trunc('minute', read_time)  = enddatetime 
                             GROUP BY project_id, read_time 
                             --ORDER BY read_time DESC
                             --LIMIT 1 
                             ) AS br ON mr.project_id = br.project_id
